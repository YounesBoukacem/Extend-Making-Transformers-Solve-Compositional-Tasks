...
# Set the device ids
deviceids = [0, 1] # Set the device ids to use for training
...
def get_lr(it):
	...
	it_period = it // period_iters
	i = 0
	it_period -= lr_schedule_periods[i]
	while it_period >= 0:
		i += 1
		it_period -= lr_schedule_periods[i]
	
	lr_schedule_period = lr_schedule_periods[i] * period_iters
	warmup_iters = lr_schedule_period * warmup_iters_ratio
	lr_decay_iters = lr_schedule_period * lr_decay_iters_ratio

	it = it - sum(lr_schedule_periods[:i]) * period_iters
...
# Create the model
model = GPT()
model.load_state_dict(torch.load("../archive/a2-push-nova/e19-push-equinox/checkpoints/best-model.pth", map_location=device))
